...
export default class CustomerController {
    ...
    async getPopularSubServices(req, res, next) {
        let subServices = await servicesServ.getPopularSubServices({service_id: req.query.service_id, limit: Number(req.query.limit ?? 10), offset: Number(req.query.offset ?? 0)})
        next(subServices)
    }
}